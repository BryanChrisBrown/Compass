...
def init_calibration(calibration_file, image_size) :
    ...
    np.savez("cameraCalibration_cv2.npz", leftCameraMatrix  = cameraMatrix_left,  leftDistCoeffs   = distCoeffs_left, 
                                                 rightCameraMatrix = cameraMatrix_right, rightDistCoeffs  = distCoeffs_right, 
                                                 R1 = R1, R2 = R2, baseline = np.linalg.norm(T))
...
def main() :
    ...
    if len(sys.argv) == 1 :
        print('python zedCalibration.py ZED_serial_number [camera_index]')
        exit(1)

    # Open the ZED camera
    cap = cv2.VideoCapture(int(sys.argv[2]) if len(sys.argv) > 2 else 0)
    if cap.isOpened() == 0:
        exit(-1)
